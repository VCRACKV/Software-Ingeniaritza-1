...
public class Main extends JFrame {
	...
	private DefaultComboBoxModel<OfferContainer> offerInfo = new DefaultComboBoxModel<OfferContainer>();

	private Collection<OfferContainer> coleccion;

	private OfferManager businessLogic;  //  @jve:decl-index=0:
	private JScrollPane offerListScrollPane = null;
	private JLabel monthLabel = null;
	private JLabel dayLabel = null;
	private JLabel searchResult = null;

	private OfferContainer selectedOffer;
	private JComboBox<OfferContainer> offerBox;
	/**
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			//jContentPane.add(getOfferListScrollPane(), null);
			...
			lookforOffers.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					try{
						...
						MyOfferManager offerm = new MyOfferManager();
						coleccion = offerm.getConcreteOfferContainer(city.getText(), date);
						for (OfferContainer v : coleccion)  offerInfo.addElement(v); 
						...
						searchResult.setText("You must write a correct date!");
					}
		...
		if (bookOffer== null) {
			...
			bookOffer.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					int num=0;
					boolean error=false;
					RuralHouse_objectdbAccess datubasea = new RuralHouse_objectdbAccess();
					if (tripleRooms.isSelected()) { 						
						num=datubasea.updateTriple(selectedOffer.getOffer().getOfferNumber());
						if(num<0)error=true;
					}
					else if (doubleRooms.isSelected()) {
						num=datubasea.updateDouble(selectedOffer.getOffer().getOfferNumber());
						if(num<0)error=true;
					}
					else if (singleRooms.isSelected()) {
						num=datubasea.updateSingle(selectedOffer.getOffer().getOfferNumber());
						if(num<0)error=true;
					}
					if (error) bookOffer.setText("Error: There were no offers available!");
					else bookOffer.setText("Booked. #rooms left: "+(num));
					bookOffer.setEnabled(false);
					datubasea.close();
				}
			});
	...
	/**
	 * This method initializes offerListScrollPane	
	 * 	
	...
	public static void main(String[] args)throws Exception {
		// TODO Auto-generated method stub
		 ...
		 QName qname = new QName("http://businessLogic/","MyOfferManagerService");
		 Service service = Service.create(url, qname);
		OfferManager wsl=
		 service.getPort(OfferManager.class);
		Main p = new Main();
		p.setBusinessLogic(wsl);
		p.setVisible(true);
	...
	private JComboBox<OfferContainer> getOfferBox() {
		if (offerBox == null) {
			offerBox = new JComboBox<OfferContainer>();
			...
			offerBox.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(offerBox.getItemCount()!=0){
						selectedOffer = (OfferContainer) offerBox.getSelectedItem();
						int num=0;
						boolean error=false;
						if (tripleRooms.isSelected()) { 
							num=selectedOffer.getOffer().getTripleNumber();
							if(num==0) error=true; else error=false;
						}
						else if (doubleRooms.isSelected()) {
							num=selectedOffer.getOffer().getDoubleNumber();
							if(num==0) error=true; else error=false;
						}
						else if (singleRooms.isSelected()) {
							num=selectedOffer.getOffer().getSingleNumber();
							if(num==0) error=true; else error=false;
						}
						if(error==true){
							bookOffer.setText("No available offers for this room type."); 
							bookOffer.setEnabled(false);
						} else{
