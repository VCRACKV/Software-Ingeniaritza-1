...
public class MyOfferManager implements OfferManager{
	...
	@WebMethod
	public Collection<OfferContainer> getConcreteOfferContainer(String city, Date date) {
		RuralHouse_objectdbAccess dbRH= new RuralHouse_objectdbAccess();
	
		ArrayList<OfferContainer> res = new ArrayList<OfferContainer>();
		for (RuralHouse rh : dbRH.getRuralHouses()) 
			if ((rh.getCity().equals(city)))
				for (Offer off : rh.getOffers()) 
					if ((off.getDate().compareTo(date))==0) 
						res.add(new OfferContainer (off));
		dbRH.close();
		return res;	
	}
	...
	@WebMethod
	public void updateSingle(int offerN){
		RuralHouse_objectdbAccess dbRH= new RuralHouse_objectdbAccess();
		dbRH.updateSingle(offerN);
		dbRH.close();

	}
	@WebMethod
	public void updateDouble(int offerN){
		RuralHouse_objectdbAccess dbRH= new RuralHouse_objectdbAccess();
		dbRH.updateDouble(offerN);
		dbRH.close();

	}
	@WebMethod
	public void updateTriple(int offerN){
		RuralHouse_objectdbAccess dbRH= new RuralHouse_objectdbAccess();
		dbRH.updateTriple(offerN);
		dbRH.close();
